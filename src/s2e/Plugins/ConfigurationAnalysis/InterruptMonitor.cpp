//
// Created by yigonghu on 9/30/19.
//
#include <vector>
#include <map>
...
#include <s2e/Plugins/ConfigurationAnalysis/InterruptMonitor.h>
...
namespace plugins {
        ...
        void InterruptMonitor::slotTranslateBlockEnd(ExecutionSignal *signal,S2EExecutionState *state,TranslationBlock *tb, uint64_t pc, bool, uint64_t){
            if (tb->se_tb_type == TB_IRET){
                signal->connect(sigc::mem_fun(*this, &InterruptMonitor::onInterrupt));
            }
        }

        void InterruptMonitor::onTranslateJumpStart(ExecutionSignal *signal,S2EExecutionState *state,TranslationBlock * tb,uint64_t pc, int jump_type){
        ...
        void InterruptMonitor::onInterruptReturn(S2EExecutionState* state, uint64_t pc){
            ...
            if (!state->readMemoryConcrete(esp, &eip, sizeof(eip), S2EExecutionState::VirtualAddress))
                //s2e()->getWarningsStream() << "IRET at 0x" << std::hex << pc << " has symbolic EIP value at memory address 0x" << hexval(esp) << std::endl;

            ReturnSignalsMap::iterator itr = plgState->m_returnSignals.find(eip);

            if (itr != plgState->m_returnSignals.end()){
                if (itr->second.empty())
                    //s2e()->getWarningsStream() << "Vector of signals was empty when trying to find interrupt for IRET to 0x" << hexval(eip) << std::endl;
                else {
        ...
        void InterruptMonitor::onInterrupt(S2EExecutionState* state, uint64_t pc) {
            ...
            if (!state->readMemoryConcrete(pc, &insnByte, 1)){
               // s2e()->getWarningsStream() << "Could not read interrupt instruction at 0x" << hexval(pc)<< std::endl;
                return;
            }
            ...
            if ((insnByte & 0xFF) == 0xCC){
                ...
                if (!state->readMemoryConcrete(pc + 1, &intNumByte, 1)){
                    //s2e()->getWarningsStream() << "Could not read interrupt index at 0x" << hexval(pc)  << std::endl;
                    return;
                }
                ...
                /* Invalid Opcode */
             //   s2e()->getWarningsStream() << "Unexpected opcode 0x" << hexval((unsigned int) insnByte) << " at 0x" <<pc << ", expected 0xcc or 0xcd"  << std::endl;
                return;
            }
